...
ggplot(dat.res,aes(x=fit,y=res,alpha=0.1))+
  ...
  geom_line(data=subset(dat.res,sample%in%relm[relm$p=='N','sample']),
            ...
            formula=y~x,se=F,size=0.5,alpha=0.2)+
  geom_line(data=subset(dat.res,sample%in%relm[relm$p=='Y','sample']),
            aes(group=sample),stat='smooth',method='lm',
            formula=y~x+I(x^2),se=F,size=0.5,alpha=0.2)+
  geom_hline(yintercept = 0,linetype=2)+scale_x_log10()

hist(dat.res$sdres,breaks = 76,xlab="Standardized residuals",
     main="Histogram of standardized residuals",family='serif')
qqnorm(dat.res$sdres,main = "Normal Q-Q Plot of standardized residuals",
       family="serif");qqline(dat.res$sdres)
...
ggplot(data=dat.ci2019,aes(x=(indi),y=(copy)))+
  stat_smooth(method="lm",formula=y~x,col="black")+
  geom_point()+
...
#bray-curtis between different surveys
rm(list = ls()[ls()!='treefish2'])
library(vegan)
library(ggplot2)
dat.c<-read.csv("MHS_capture.csv",row.names = 1)
...
dat.e[rownames(dat.e) %in% 
        dat.ei[dat.ei$year==2019,'sample'],]|>colSums()->e2019
dat.e[rownames(dat.e) %in% 
        dat.ei[dat.ei$year==2020,'sample'],]|>colSums()->e2020
t(dat.c)|>merge(cbind(e2019,e2020),by="row.names",all=T)->dat.m
row.names(dat.m)<-dat.m[,1]
dat.m<-dat.m[,-1]
dat.m[is.na(dat.m)]<-0
dat.m<-t(dat.m)

decostand(dat.m,method = 'total')|>vegdist(method="bray")|>
  as.matrix()|>round(3)->dat.veg

len<-length(colnames(dat.veg))
dat.p.veg<-c()
for (i in 1:len) {
  dat.p.vegb<-data.frame(x=colnames(dat.veg)[i],
                         y=rownames(dat.veg)[i:len],
                         dis=dat.veg[i:len,i])
  dat.p.veg<-rbind(dat.p.veg,dat.p.vegb)
}

lev<-c("y2007","y2008","y2009","y2010","y2019","e2019","e2020")
lev2<-c("e2020","e2019","y2019","y2010","y2009","y2008","y2007")
dat.p.veg$x<-factor(dat.p.veg$x,levels = lev)
dat.p.veg$y<-factor(dat.p.veg$y,levels = lev2)
lab<-c("y2007"="T\n2007","y2008"="T\n2008",
       "y2009"="T\n2009","y2010"="T\n2010","y2019"="T\n2019",
       "e2019"="e\n2019","e2020"="e\n2020")
ggplot(dat.p.veg,aes(x=x,y=y))+
  geom_tile(aes(fill=dis))+
  geom_text(aes(label=dis),family="serif")+
  scale_fill_gradient(low="white",high="grey",name="Bray-Curtis")+
...
te<-data.frame(res=modGS$residuals,
               ...
               response=modGS$y)
ggplot(te,aes(x=lip,y=res))+geom_point(alpha=0.3)+
  geom_smooth(method='lm',formula=y~x)+
  geom_hline(yintercept = 0,linetype=2)+
  theme_bw()+
  xlab('Linear predictors')+ylab('Residuals')+
...
ggplot(te,aes(x=res))+
  geom_histogram(fill='grey',stat='bin',binwidth = 0.25,
                 col='black',alpha=0.5)+
  theme_bw()+xlab('Residuals')+ylab('Frequency')+
  theme(text=element_text(size=20,family="serif"))

qqnorm(te$res,main = "Normal Q-Q Plot of residuals",
       family="serif");qqline(te$res)

ggplot(te,aes(x=lip,y=response))+geom_point(alpha=0.3)+
  geom_abline(slope=1,linetype=2)+
  theme_bw()+xlab('Fitted Values')+ylab('Response')+
  theme(panel.grid=element_blank(),
        text=element_text(size=20,family="serif"))
